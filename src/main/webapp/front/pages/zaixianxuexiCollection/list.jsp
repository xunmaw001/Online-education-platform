            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">在线学习收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="zaixianxuexiName" id="zaixianxuexiName" placeholder="标题" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="kechngTypes" id="kechngTypes" lay-filter="kechngTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in kechngTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../zaixianxuexi/detail.jsp?id='+item.zaixianxuexiId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.zaixianxuexiPhoto?item.zaixianxuexiPhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../zaixianxuexi/detail.jsp?id='+item.zaixianxuexiId)"><span>{{item.zaixianxuexiName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.newMoney"><span style="color: red;">{{item.newMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteZaixianxuexiCollection(item.id)">删除 {{item.zaixianxuexiName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 zaixianxuexiCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            zaixianxuexiCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteZaixianxuexiCollection(id){
                var paramArray = [];
                paramArray.push(id);
                layui.http.requestJson(`zaixianxuexiCollection/delete`, 'post',paramArray, function (res) {
                    if (res.code == 0) {
                        layer.msg('删除成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#zaixianxuexiName').val() != null && jquery('#zaixianxuexiName').val() != "" && jquery('#zaixianxuexiName').val() != "null" ) {
                param['zaixianxuexiName'] = jquery('#zaixianxuexiName').val();
            }
                                 
            var kechngTypes = $("#kechngTypes").find("option:selected").val();
            if (kechngTypes != null && kechngTypes != "" && kechngTypes != "null") {
                            param['kechngTypes'] = kechngTypes;
                        }
                                                                
            // 当前表
                                // 获取列表数据
            http.request('zaixianxuexiCollection/list', 'get', param, function(res) {
...
// 级联表的字典表
       ...
       function kechngTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=kechng_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.kechngTypesList = res.data.list;
               }
           });
       }
