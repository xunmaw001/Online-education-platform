...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("messages/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addmessages');
    ...
    // 获取当前详情
    function getDetails() {
        var addmessages = window.sessionStorage.getItem("addmessages");
        if (addmessages != null && addmessages != "" && addmessages != "null") {
            window.sessionStorage.removeItem('addmessages');
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("messages/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
